...
def check_server_status(key, host_addr, group):
    ...
    for i, row in host_addr.iterrows():
        ...
        try:
            ...
            backup_dir = "/root/log/" + group + "/status"
            ...
            print("step1-1")
            df = run_server_cmd(id,df,h,group,c,"sudo service httpd status","step1-2")
            print("step1-2")
            df = run_server_cmd(id,df,h,group,c,"sudo service iptables status","step1-3")
            print("step1-3")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":80 --connect-timeout 5 -sS","step1-4")
            print("step1-4")

            df = run_server_cmd(id,df,h,group,c,"sudo tail -n 3 /var/log/httpd/access_log","step2")
            print("step2")

            df = run_server_cmd(id,df,h,group,c,"sudo find /var/www/html/ensyu.txt","step3-1")
            print("step3-1")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":80/ensyu.txt --connect-timeout 5 -sS","step3-2")
            print("step3-2")

            #df = run_server_cmd(id,df,h,group,c,"sudo find /var/www/rootdirectory","step4-1")
            #print("step4-1")
            df = run_server_cmd(id,df,h,group,c,"sudo find /var/www/rootdirectory/ensyu.txt","step4-2")
            print("step4-2")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":80/ensyu.txt --connect-timeout 5 -sS","step4-3")
            print("step4-3")

            df = run_server_cmd(id,df,h,group,c,"sudo cat /etc/httpd/conf/httpd.conf | grep Listen","step5-1")
            print("step5-1")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":443 --connect-timeout 5 -sS","step5-2")
            print("step5-2")

            df = run_server_cmd(id,df,h,group,c,"sudo cat /etc/httpd/conf/httpd.conf | grep DirectoryIndex","step6-1")
            print("step6-1")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":443 --connect-timeout 5 -sS","step6-2")
            print("step6-2")

            df = run_server_cmd(id,df,h,group,c,"sudo service iptables status","step7-1")
            print("step7-1")
            df = run_local_cmd(id,df,h,group,c,"curl http://"+h+":443 --connect-timeout 5 -sS","step7-2")
            print("step7-2")

            df = run_server_cmd(id,df,h,group,c,"sudo yum list installed | grep samba","step8-1")
            print("step8-1")
            df = run_server_cmd(id,df,h,group,c,"sudo pdbedit -L","step8-2")
            print("step8-2")
            df = run_server_cmd(id,df,h,group,c,"sudo service samba status","step8-3")
            print("step8-3")
            df = run_server_cmd(id,df,h,group,c,"sudo chkconfig smb --list","step8-4")
            print("step8-4")
...
if __name__ == '__main__':
    ...
    try:
        ...
        df.to_csv("/root/log/"+group+"/status/"+record_date+".tsv",sep='\t', encoding='utf-8',quotechar='\'')
