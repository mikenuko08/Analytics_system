...
def get_git_logs(key, host_addr, group):
    ...
    for i, row in enumerate(host_addr):
        ...
        try:
            ...
            c = Connection(host=h,user="root",port=22,connect_timeout=2,connect_kwargs={"key_filename":key})
...
def get_all_logs(key, host_addr, group):
    ...
    for i, row in host_addr.iterrows():
        ...
        try:
            print("--------------- Start get_all_logs function "+id.zfill(3)+" ---------------")
            c = Connection(host=h,user="logger",port=22,connect_timeout=5,connect_kwargs={"key_filename":key})
            ...
            c.run("sudo tar czf "+logger_dir+"/script.tar.gz -C /var/log/ script", warn=True)
            print("Created script.tar.gz on remote")
            
            c.get("/home/logger/log/script.tar.gz",backup_dir+"/script.tar.gz")
            print("Get script.tar.gz on remote")
            ...
            print("Copy /root/.command_history to /home/logger/log/.command_history on remote")
            
            c.get(logger_dir+"/.command_history", backup_dir+"/.command_history")
            print("Get .command_history on remote")

            c.run("sudo tar czf "+logger_dir+"/git.tar.gz"+" -C / git", warn=True)
